...
function renderHome() {
  ...
  $("homePage").innerHTML = `
    ...
    ${renderGroupHeader("Summary", "", "Home")}
    <div class="grid two">
      <article class="card">
        <h3>Drive and electrical</h3>
        ${renderKeyValueList([...(overview?.drive || []).slice(0, 5), ...(overview?.electrical || []).slice(0, 4)], "homeDriveElectrical")}
      </article>
      <article class="card">
        <h3>Process and pump</h3>
        ${renderKeyValueList([...(overview?.process || []).slice(0, 5), ...(overview?.pump || []).slice(0, 4)], "homeProcessPump")}
      </article>
    </div>
    <div class="grid two">
      <article class="card">
        <h3>Diagnostics</h3>
        ${renderKeyValueList((overview?.diagnostics || []).slice(0, 5), "homeDiagnostics")}
      </article>
      <article class="card">
        <h3>I/O snapshot</h3>
        ${renderKeyValueList((overview?.io || []).slice(0, 8), "homeIo")}
      </article>
    </div>
  `;
}
  ...
  $("monitoringPage").innerHTML = `
    <div class="group-shell">
      ...
      <div class="grid three">
        <article class="card">
          <h3>State and channels</h3>
          ${renderKeyValueList(overview.drive, "monitorDrive")}
        </article>
        <article class="card">
        ...
        <article class="card">
          <h3>I/O snapshot</h3>
          ${renderKeyValueList(overview.io, "monitorIo")}
        </article>
      </div>
      ${renderGroupHeader("PID and Process")}
      <article class="card">
        <h3>Process values</h3>
        ${renderKeyValueList(overview.process, "monitorProcess")}
      </article>
      ${renderGroupHeader("Energy and Efficiency")}
      <article class="card">
        <h3>Energy values</h3>
        ${renderKeyValueList(overview.energy, "monitorEnergy")}
      </article>
      ${renderGroupHeader("Pump and Multipump")}
      <article class="card">
        <h3>Pump values</h3>
        ${renderKeyValueList(overview.pump, "monitorPump")}
      </article>
      ${renderGroupHeader("Diagnostics")}
      <article class="card">
        <h3>Faults and diagnostics</h3>
        ${renderKeyValueList(overview.diagnostics, "monitorDiagnostics")}
      </article>
      <article class="card">
        <div class="card-head">
          <div>
...
function updateHomeValues() {
  ...
  patchLiveItems("homeDriveElectrical", [...(overview.drive || []).slice(0, 5), ...(overview.electrical || []).slice(0, 4)]);
  patchLiveItems("homeProcessPump", [...(overview.process || []).slice(0, 5), ...(overview.pump || []).slice(0, 4)]);
  patchLiveItems("homeDiagnostics", (overview.diagnostics || []).slice(0, 5));
  patchLiveItems("homeIo", (overview.io || []).slice(0, 8));
}
...
function updateMonitoringValues() {
  ...
  patchLiveItems("monitorDrive", overview.drive || []);
  patchLiveItems("monitorElectrical", overview.electrical || []);
  patchLiveItems("monitorIo", overview.io || []);
  patchLiveItems("monitorProcess", overview.process || []);
  patchLiveItems("monitorEnergy", overview.energy || []);
  patchLiveItems("monitorPump", overview.pump || []);
  patchLiveItems("monitorDiagnostics", overview.diagnostics || []);
